...
string osmlink = "", osmpath = "";
vector<string> swear_words = {}, splicing, combining, combusting, crystals;
bool debug = false;
namespace servers {
	/* newbie award */
	map<int, int> newbie_award;
	int newbieGems = 0;
	/* exchange */
	bool isActive = false;
	vector<vector<pair<int, int>>> exchange;
	/* upgrade system */
	bool isOpen = true;
	vector<vector<pair<int, int>>> itemlist;
	/* punch */
	map<int, int> punch;
	/* item adjustment */
	map<int, int> gems;
	map<int, int> fars;
	map<int, int> breakhits;
	map<int, int> exp;
	/* block adjustment */
	map<int, int> maxgems;
	/* sellfish */
...
ENetHost* server;
string documents = "C:/Users/Admin/Desktop/SERVER", ipvps = "";
...
vector<vector<string>> info_about_playmods{
	...
	{ "121", "2480", "250", "VIP SB!", "Broadcasting to ALL!", "You can broadcast once again.", "2480", "", "", "", "", "", "", "" },
	{ "122", "2480", "720", "SPECIAL SB!", "Broadcasting to ALL!", "You can broadcast once again.", "2480", "", "", "", "", "", "", "" },
	...
	{ "144", "5926", "250", "Ultra World Spray", "Calmdown with the world spray..", "You can now use ultra world spray again.", "5926", "", "", "", "", "", "", "" },
	...
	{ "155", "2480", "250", "MOD SB!", "Broadcasting to ALL!", "You can broadcast once again.", "2480", "", "", "", "", "", "", "" },
	{ "156", "2480", "250", "DEV SB!", "Broadcasting to ALL!", "You can broadcast once again.", "2480", "", "", "", "", "", "", "" },
	{ "157", "2480", "250", "SDEV SB!", "Broadcasting to ALL!", "You can broadcast once again.", "2480", "", "", "", "", "", "", "" }
};
...
namespace base64 {
	...
	std::string to_base64(const std::string& input) {
		...
		for (size_t in_ = 0; in_ < input.size(); in_++) {
...
int get_punch_id(const int id_) {
	if (!servers::punch.contains(id_)) return 0;

	return servers::punch[id_];
}
...
// Helper functions for items.dat conversion
inline void write_buffer_number(vector<BYTE>& buffer, int& pos, int len, int64_t value) {
	for (int a = 0; a < len; a++) {
		buffer[pos + a] = (value >> (a * 8)) & 0xFF;
	}
	pos += len;
}

inline void write_buffer_string(vector<BYTE>& buffer, int& pos, const string& value, bool using_key = false, int item_id = 0) {
	const string key = "PBG892FXX982ABC*";
	int16_t len = (int16_t)value.length();
	// Write length first (2 bytes)
	buffer[pos] = len & 0xFF;
	buffer[pos + 1] = (len >> 8) & 0xFF;
	pos += 2;
	// Write string data
	for (int a = 0; a < len; a++) {
		if (using_key) {
			buffer[pos + a] = value[a] ^ key[(a + item_id) % key.length()];
		} else {
			buffer[pos + a] = value[a];
		}
	}
	pos += len;
}

inline void write_buffer_bytes(vector<BYTE>& buffer, int& pos, const vector<int>& data) {
	for (size_t a = 0; a < data.size(); a++) {
		buffer[pos + a] = (BYTE)(data[a] & 0xFF);
	}
	pos += (int)data.size();
}

// Convert items.json to items.dat and return the binary data
bool convert_json_to_items_dat(const string& json_path, const string& output_path) {
	// Read JSON file
	ifstream json_file(json_path);
	if (!json_file.is_open()) {
		cout << "Failed to open items.json at: " << json_path << endl;
		return false;
	}
	
	json items_json;
	try {
		json_file >> items_json;
	} catch (const exception& e) {
		cout << "Failed to parse items.json: " << e.what() << endl;
		return false;
	}
	json_file.close();
	
	if (!items_json.is_array()) {
		cout << "items.json must be an array" << endl;
		return false;
	}
	
	int item_count = (int)items_json.size();
	int version = 15; // Default version, will detect from data
	
	// First pass: calculate buffer size
	size_t buffer_size = 6; // version (2) + item_count (4)
	for (int a = 0; a < item_count; a++) {
		const auto& item = items_json[a];
		buffer_size += 4; // item_id
		buffer_size += 4; // editable_type, item_category, action_type, hit_sound_type
		buffer_size += 2 + item.value("name", "").size(); // name len + name
		buffer_size += 2 + item.value("texture", "").size(); // texture len + texture
		buffer_size += 4; // texture_hash
		buffer_size += 1; // item_kind
		buffer_size += 4; // val1
		buffer_size += 5; // texture_x, texture_y, spread_type, is_stripey_wallpaper, collision_type
		buffer_size += 1; // break_hits
		buffer_size += 4; // drop_chance
		buffer_size += 1; // clothing_type
		buffer_size += 2; // rarity
		buffer_size += 1; // max_amount
		buffer_size += 2 + item.value("extra_file", "").size(); // extra_file len + extra_file
		buffer_size += 4; // extra_file_hash
		buffer_size += 4; // audio_volume
		buffer_size += 2 + item.value("pet_name", "").size(); // pet_name
		buffer_size += 2 + item.value("pet_prefix", "").size(); // pet_prefix
		buffer_size += 2 + item.value("pet_suffix", "").size(); // pet_suffix
		buffer_size += 2 + item.value("pet_ability", "").size(); // pet_ability
		buffer_size += 4; // seed_base, seed_overlay, tree_base, tree_leaves
		buffer_size += 8; // seed_color (4) + seed_overlay_color (4)
		buffer_size += 4; // ingredients
		buffer_size += 4; // grow_time
		buffer_size += 2; // val2
		buffer_size += 2; // is_rayman
		buffer_size += 2 + item.value("extra_options", "").size(); // extra_options
		buffer_size += 2 + item.value("texture2", "").size(); // texture2
		buffer_size += 2 + item.value("extra_options2", "").size(); // extra_options2
		buffer_size += 80; // data_position_80
		// Version 11+
		buffer_size += 2 + item.value("punch_options", "").size();
		// Version 12+
		buffer_size += 13;
		// Version 13+
		buffer_size += 4;
		...
		// Version 15+
		buffer_size += 25;
		buffer_size += 2 + item.value("str_version_15", "").size();
	}
	
	// Allocate buffer
	vector<BYTE> buffer(buffer_size + 1000, 0); // Extra padding for safety
	int mem_pos = 0;
	
	// Write header: version (2 bytes) + item_count (4 bytes)
	write_buffer_number(buffer, mem_pos, 2, version);
	write_buffer_number(buffer, mem_pos, 4, item_count);
	...
	for (int a = 0; a < item_count; a++) {
		...
		// item_id (4 bytes)
		int item_id = item.value("id", a);
		write_buffer_number(buffer, mem_pos, 4, item_id);
		
		// editable_type (1 byte)
		buffer[mem_pos++] = (BYTE)item.value("editable_type", 0);
		// item_category (1 byte)
		buffer[mem_pos++] = (BYTE)item.value("item_category", 0);
		// action_type (1 byte)
		buffer[mem_pos++] = (BYTE)item.value("action_type", 0);
		// hit_sound_type (1 byte)
		buffer[mem_pos++] = (BYTE)item.value("hit_sound_type", 0);
		
		// name (encrypted with key)
		string name = item.value("name", "");
		write_buffer_string(buffer, mem_pos, name, true, item_id);
		
		// texture
		string texture = item.value("texture", "");
		write_buffer_string(buffer, mem_pos, texture, false);
		
		// texture_hash (4 bytes)
		write_buffer_number(buffer, mem_pos, 4, item.value("texture_hash", 0));
		
		// item_kind (1 byte)
		buffer[mem_pos++] = (BYTE)item.value("item_kind", 0);
		
		// val1 (4 bytes)
		write_buffer_number(buffer, mem_pos, 4, item.value("val1", -1));
		
		// texture_x, texture_y, spread_type, is_stripey_wallpaper, collision_type (1 byte each)
		buffer[mem_pos++] = (BYTE)item.value("texture_x", 0);
		buffer[mem_pos++] = (BYTE)item.value("texture_y", 0);
		buffer[mem_pos++] = (BYTE)item.value("spread_type", 0);
		buffer[mem_pos++] = (BYTE)item.value("is_stripey_wallpaper", 0);
		buffer[mem_pos++] = (BYTE)item.value("collision_type", 0);
		
		// break_hits (1 byte) - stored as raw value in JSON
		int break_hits = item.value("break_hits", 0);
		buffer[mem_pos++] = (BYTE)(break_hits & 0xFF);
		
		// drop_chance (4 bytes)
		write_buffer_number(buffer, mem_pos, 4, item.value("drop_chance", 0));
		
		// clothing_type (1 byte)
		buffer[mem_pos++] = (BYTE)item.value("clothing_type", 0);
		
		// rarity (2 bytes)
		write_buffer_number(buffer, mem_pos, 2, item.value("rarity", 0));
		
		// max_amount (1 byte)
		buffer[mem_pos++] = (BYTE)item.value("max_amount", 200);
		
		// extra_file
		string extra_file = item.value("extra_file", "");
		write_buffer_string(buffer, mem_pos, extra_file, false);
		
		// extra_file_hash (4 bytes)
		write_buffer_number(buffer, mem_pos, 4, item.value("extra_file_hash", 0));
		
		// audio_volume (4 bytes)
		write_buffer_number(buffer, mem_pos, 4, item.value("audio_volume", 400));
		
		// pet_name, pet_prefix, pet_suffix, pet_ability
		write_buffer_string(buffer, mem_pos, item.value("pet_name", ""), false);
		write_buffer_string(buffer, mem_pos, item.value("pet_prefix", ""), false);
		write_buffer_string(buffer, mem_pos, item.value("pet_suffix", ""), false);
		write_buffer_string(buffer, mem_pos, item.value("pet_ability", ""), false);
		
		// seed_base, seed_overlay, tree_base, tree_leaves (1 byte each)
		buffer[mem_pos++] = (BYTE)item.value("seed_base", 0);
		buffer[mem_pos++] = (BYTE)item.value("seed_overlay", 0);
		buffer[mem_pos++] = (BYTE)item.value("tree_base", 0);
		buffer[mem_pos++] = (BYTE)item.value("tree_leaves", 0);
		
		// seed_color (4 bytes as uint32)
		uint32_t seed_color = item.value("seed_color", (uint32_t)4294967295);
		write_buffer_number(buffer, mem_pos, 4, seed_color);
		
		// seed_overlay_color (4 bytes as uint32)
		uint32_t seed_overlay_color = item.value("seed_overlay_color", (uint32_t)4294967295);
		write_buffer_number(buffer, mem_pos, 4, seed_overlay_color);
		
		// ingredients (4 bytes - skip/set to 0)
		write_buffer_number(buffer, mem_pos, 4, 0);
		
		// grow_time (4 bytes)
		write_buffer_number(buffer, mem_pos, 4, item.value("grow_time", 0));
		
		// val2 (2 bytes)
		write_buffer_number(buffer, mem_pos, 2, item.value("val2", 0));
		
		// is_rayman (2 bytes)
		write_buffer_number(buffer, mem_pos, 2, item.value("is_rayman", 0));
		
		// extra_options, texture2, extra_options2
		write_buffer_string(buffer, mem_pos, item.value("extra_options", ""), false);
		write_buffer_string(buffer, mem_pos, item.value("texture2", ""), false);
		write_buffer_string(buffer, mem_pos, item.value("extra_options2", ""), false);
		
		// data_position_80 (80 bytes)
		if (item.contains("data_position_80") && item["data_position_80"].is_array()) {
			vector<int> data80 = item["data_position_80"].get<vector<int>>();
			data80.resize(80, 0);
			write_buffer_bytes(buffer, mem_pos, data80);
		} else {
			mem_pos += 80;
		}
		
		// Version 11+: punch_options
		if (version >= 11) {
			write_buffer_string(buffer, mem_pos, item.value("punch_options", ""), false);
		}
		
		// Version 12+: data_version_12 (13 bytes)
		if (version >= 12) {
			if (item.contains("data_version_12") && item["data_version_12"].is_array()) {
				vector<int> data12 = item["data_version_12"].get<vector<int>>();
				data12.resize(13, 0);
				write_buffer_bytes(buffer, mem_pos, data12);
			} else {
				mem_pos += 13;
			}
		}
		
		// Version 13+: int_version_13 (4 bytes)
		if (version >= 13) {
			write_buffer_number(buffer, mem_pos, 4, item.value("int_version_13", 0));
		}
		
		// Version 14+: int_version_14 (4 bytes)
		if (version >= 14) {
			write_buffer_number(buffer, mem_pos, 4, item.value("int_version_14", 0));
		}
		
		// Version 15+: data_version_15 (25 bytes) + str_version_15
		if (version >= 15) {
			if (item.contains("data_version_15") && item["data_version_15"].is_array()) {
				vector<int> data15 = item["data_version_15"].get<vector<int>>();
				data15.resize(25, 0);
				write_buffer_bytes(buffer, mem_pos, data15);
			} else {
				mem_pos += 25;
			}
			write_buffer_string(buffer, mem_pos, item.value("str_version_15", ""), false);
		}
	}
	...
	if (!out_file.is_open()) {
		cout << "Failed to create items.dat at: " << output_path << endl;
		return false;
	}
	out_file.write((char*)buffer.data(), mem_pos);
	out_file.close();
	
	cout << "Successfully converted items.json to items.dat (" << item_count << " items, " << mem_pos << " bytes)" << endl;
	return true;
}
...
int items_dat() {
	...
	// Convert items.json to items.dat first
	string json_path = "db/items/items.json";
	string dat_path = "db/items/items.dat";
	convert_json_to_items_dat(json_path, dat_path);
	...
	//	if (def.actionType == 98) cout << def.id << endl;
		...
		if (items[i].xmlFile != "" && items[i].clothType == ClothTypes::HAND) items[i].effect = 237;
		if (items[i].xmlFile != "" && items[i].clothType == ClothTypes::FEET) items[i].effect = 244;
		if (servers::maxgems.contains(i) && servers::maxgems[i] != 0) items[i].max_gems = servers::maxgems[i];
		...
		if (server_port == 9999) items[i].gtwl *= 1000;
		if (items[i].gtwl != 0) {
			items[i].gtwl *= 1.1;
		...
		if (i == 9780 || i == 11168 || i == 13426 || i == 5926) items[i].oprc = 450;
		...
		if (i == 12646 || i == 12648 || i == 12650 || i == 11464 || i == 11466 || i == 11470 || i == 9470 || i== 5926 || i == 2722 || i == 12876|| i == 11882 || i == 6948 || i == 1836 || i == 11562 || i == 12014 || i == 12016 || i == 12018 || i == 12020 || i == 12374 || i == 12422 || i == 12424 || i == 12426 || i == 12428)  items[i].pwl = 500;
	...
	if (file_info.is_open()) {
		...
		while (getline(file_info, line_info)) {
			...
			if (d_[6] != "" && items[atoi(d_[0].c_str())].description != "No Info") items[atoi(d_[0].c_str())].description = d_[6];
	...
	for (int i = 0; i < itemCount; i++) {
		...
		if (items[i].blockType == CLOTHING && items[i].unobtainable == false && i != 1780 && i != 19558 && items[i].untradeable == 0 && i != 22268 && to_lower(items[i].name).find("pickaxe tier") != 0
			&& to_lower(items[i].name).find("kyub") == string::npos && to_lower(items[i].name).find("susano") == string::npos
			&& to_lower(items[i].name).find("mario") == string::npos && to_lower(items[i].name).find("shikifujin") == string::npos) dstone.push_back(i);
...
void coordinate_range(int const& range, proton::Vector2 const& source, proton::Vector2 const& punch, std::function<void(proton::Vector2 const& pos)> const& fn) {
	int sourceX = source.get_x(), sourceY = source.get_y();

	for (int i = 0; i < range; i++) {
		int x = punch.get_x(), y = punch.get_y();

		if (sourceX != x) {
			if (sourceX > x) x -= i;

			else if (sourceX < x) x += i;
		}

		if (sourceY < y) y += i;

		else if (sourceY > y) y -= i;

		fn(proton::Vector2{ static_cast<float>(x), static_cast<float>(y) });
	}
}