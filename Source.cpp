...
void loop_worlds() {
	...
	if (f_saving_ == false) {
		...
		if (last_time - ms_time <= 0) {
			...
			if (World_Stuff.last_world_menu - ms_time <= 0) {
				...
				for (uint8_t i = 0; i < (World_Stuff.top_active_worlds.size() > 20 ? 20 : World_Stuff.top_active_worlds.size()); i++) {
					...
					World_Stuff.active_world_list += "\nadd_floater|" + World_Stuff.top_active_worlds[i].second + "|" + to_string(w_cz) + "|0.4" + (i == 0 ? "5" : "2") + "|" + to_string(ColorConverter::toBGRA(blue, green, red, 255));
				}
				if (World_Stuff.active_world_list.empty()) {
					...
					for (uint8_t i = 0; i < (worlds.size() >= 18 ? 18 : worlds.size()); i++) {
						...
						if (find(active_worlds.begin(), active_worlds.end(), world_.name) == active_worlds.end()) {
							//0.5
							World_Stuff.active_world_list += "\nadd_floater|" + world_.name + "|0|0.42|" + to_string(ColorConverter::toBGRA(blue, green, red, 255));
							active_worlds.push_back(world_.name);
						}
					}
				}
				if (World_Stuff.active_world_list.empty())World_Stuff.active_world_list = "\nadd_floater|START|0|0.5|" + to_string(ColorConverter::toBGRA(blue, green, red, 255));
			...
			for (ENetPeer* currentPeer = server->peers; currentPeer < &server->peers[server->peerCount]; ++currentPeer) {
				...
				if (pInfo(currentPeer)->cheater_settings & Gtps3::SETTINGS_0 && pInfo(currentPeer)->disable_cheater == 0) {
					if (pInfo(currentPeer)->last_used_block != 0 && pInfo(currentPeer)->autofarm_x != -1) {
						int maxrange = 0, normalrange = 3;
						if (player::items(currentPeer) != -1) {
							int itemId = player::items(currentPeer);
							int farItems = player::checkfar(player::items(currentPeer));
							maxrange = farItems;
						}
						if (pInfo(currentPeer)->autofarm_slot > maxrange) (player::items(currentPeer) != -1 ? pInfo(currentPeer)->autofarm_slot = maxrange : pInfo(currentPeer)->autofarm_slot = normalrange);
						for (int i = 0; i < pInfo(currentPeer)->autofarm_slot; i++) {
							if (i > 3 && player::items(currentPeer) == -1) continue;
		...
		if (last_autofarm - ms_time <= 0) last_autofarm = ms_time + 350;
		if (last_time - ms_time <= 0) {
			...
			for (ENetPeer* currentPeer = server->peers; currentPeer < &server->peers[server->peerCount]; ++currentPeer) {
				...
				for (int i_ = 0; i_ < pInfo(currentPeer)->playmods.size(); i_++) {
					...
					if (pInfo(currentPeer)->playmods[i_].time - time_ < 0) {
						...
						if (pInfo(currentPeer)->playmods[i_].id == 126) pInfo(currentPeer)->vip = 0;
...
int main(int argc, char* argv[]) {
	...
	load_config();
	servers::load_newbie(), servers::load_itemadjustment(), servers::load_punch();
	servers::load_exchange(), servers::load_block(), servers::load_upgrade();
	system("cls");
	refresh_wotd_worlds();
	if (init_enet(server_port) == -1) cout << "enet klaida" << endl;
		...
		cout << "[Server]" << "|PORT:" << server_port << "|MAX ITEMS : " << setGems(items.size()) << "| \n" << endl;
		cout << "[INFORMATION]:" << endl;
		cout << "Loaded Newbie Award: " << servers::newbie_award.size() << endl;
		cout << "Loaded Item Far: " << servers::fars.size() << endl;
		cout << "Loaded Item Gems (x): " << servers::gems.size() << endl;
		cout << "Loaded Upgradeable items: " << servers::itemlist.size() << endl;
		cout << "Debug Xml: " << items[14084].xmlFile << endl;
	}
				...
				case 2:
				{
					...
					*/
					...
					if (pInfo(peer)->requestedName.empty()) {
						...
						//else if (msg[0] == '!' && pInfo(peer)->give_role) SendCmd2(peer, msg);
						...
						*/
						...
						break;
							...
							if (p != worlds.end()) {
								...
								WorldBlock* block_ = &world_->blocks[pInfo(peer)->lastwrenchx + (pInfo(peer)->lastwrenchy * 100)];
								if (block_->fg == 8878) {
									if (to_lower(world_->owner_name) == to_lower(pInfo(peer)->tankIDName) || pInfo(peer)->give_role) {
										vector<string> t_ = explode("|", cch);
										if (t_.size() < 4) break;
						...
						break;
							...
							if (p != worlds.end()) {
								...
								world_->fresh_world = true;
								WorldBlock* block_ = &world_->blocks[pInfo(peer)->lastwrenchx + (pInfo(peer)->lastwrenchy * 100)];
								if (to_lower(world_->owner_name) == to_lower(pInfo(peer)->tankIDName) || pInfo(peer)->give_role)load_storagebox(peer, world_, block_);
							}
							break;
						}
						...
						break;
							...
							if (id_ % 2 != 0) p.Insert("set_default_color|`o\nadd_label_with_ele_icon|big|`wAbout " + items[id_].ori_name + "``|left|" + to_string(id_) + "|" + to_string(items[id_ - 1].chi) + "|\nadd_spacer|small|\nadd_textbox|Plant this seed to grow a `0" + items[id_ - 1].ori_name + " Tree.``|left|\nadd_spacer|small|\nadd_textbox|Rarity: `0" + to_string(items[id_].rarity) + "``|left|\nadd_spacer|small|\nend_dialog|continue||OK|\n");
								...
								string extra_item = "\nadd_textbox|";
								if (servers::fars.contains(id_) && servers::fars[id_] >= 1) extra_item += "<CR>This item gives `9" + to_string(servers::fars[id_]) + " `ofar place & punch";
								if (servers::gems.contains(id_) && servers::gems[id_] >= 1) extra_item += "<CR>This item gives `9x" + to_string(servers::gems[id_]) + " `ogems";
								if (servers::exp.contains(id_) && servers::exp[id_] >= 1) extra_item += "<CR>This item gives `9x" + to_string(servers::exp[id_]) + " `oxp";
								extra_item += "|left|";
								if (extra_item == "\nadd_textbox||left|") extra_item = "";
								else extra_item = replace_str(extra_item, "add_textbox|<CR>", "add_textbox|");

								string extra_ore = "";
								if (id_ == 9386) extra_ore = rare_text;
								if (id_ == 5136) extra_ore = rainbow_text;
								p.Insert("set_default_color|`o\nadd_label_with_ele_icon|big|`wAbout " + items[id_].name + "``|left|" + to_string(id_) + "|" + to_string(items[id_].chi) + "|\nadd_spacer|small|\ntext_scaling_string|100,000OPC|\nadd_spacer|small|\nadd_textbox|" + items[id_].description + "|left|" + (pInfo(peer)->subscriber || pInfo(peer)->supp == 2 ? "\nadd_spacer|small|\nadd_textbox|`1This item average price is " + (items[id_].price.size() == 0 ? "unknown" : to_string(item_average2(items[id_].price)) + " World Locks") + "!``|left|" : "") + "" + (extra_ore != "" ? "\nadd_spacer|small|\nadd_textbox|This item also drops:|left|" + extra_ore : "") + "" + (id_ == 8552 ? "\nadd_spacer|small|\nadd_textbox|Angelic Healings: " + to_string(pInfo(peer)->surgery_done) + "|left|" : "") + "\nadd_spacer|small|" + extra_item + extra_ + "\nadd_spacer|small|\nend_dialog|continue||OK|\n");
							}
							p.CreatePacket(peer);
							break;
						}
							...
							if (pInfo(peer)->bypass) {
								...
								if (pInfo(peer)->world.empty() && pInfo(peer)->enter_game == 1) {
									...
									if (pInfo(peer)->tankIDName.empty()) {
										gamepacket_t p(1500);
										p.Insert("OnDialogRequest");
										p.Insert("set_default_color|`0\nadd_label_with_icon|big|" + server_name + " Account System|left|3902|\nadd_smalltext|For client growtopia vesion 4.61. `0 If you dont have GrowID, click `4''Create a GrowID''|left|\nadd_spacer|small|\nadd_smalltext|Options:|left|\nadd_custom_break|\nadd_button|jenotopia_register|Get a GrowID|left|\nadd_quick_exit|\nend_dialog|jenotopia_test|||\n");
										p.CreatePacket(peer);
									}
									else {
										if (pInfo(peer)->vip) pInfo(peer)->rb = 0;
										has_playmod2(pInfo(peer), 76, 1);
										pInfo(peer)->name_color = role_power::getRole_name(peer);
										int on_ = 0;
										{
										...
										if (not has_playmod2(pInfo(peer), 143) || not has_playmod2(pInfo(peer), 153)) {
						...
						else if (cch.find("action|dialog_return\ndialog_name|jenotopia_test\nbuttonClicked|jenotopia_register") != string::npos) {
							if (pInfo(peer)->requestedName != "stempek") break;
							gamepacket_t p(250);
						...
						if (cch == "action|dialog_return\ndialog_name|world_spray\n") {
							if (has_playmod2(pInfo(peer), 144)) {
								...
								if (used_item == 5926 || used_item == 13574) {
						...
						break;
							...
							if (pInfo(peer)->give_role || pInfo(peer)->Staff) {
								string button = "", search = "";
								vector<string> t_ = explode("|", cch);
								if (t_.size() > 3) {
									button = explode("\n", t_[3])[0].c_str();
									if (button == "Empty Logs" && to_lower(pInfo(peer)->tankIDName) == "Zenhyx") Server_Security.logs.clear();
								}
								if (t_.size() > 5) {
									search = explode("\n", t_[5])[0].c_str();
									if (search == "Next page") {
						...
						if (pInfo(peer)->cheater_ or has_playmod2(pInfo(peer), 143) or has_playmod2(pInfo(peer), 153)) {
							...
							if (autofarm_slot >= 1 && autofarm_slot <= (player::items(peer) == -1 ? 3 : player::checkfar(player::items(peer))) + 1) {
					...
					/*
						...
						else if (cch.find("action|dialog_return\ndialog_name|setnewbie") != string::npos) {
							if (!pInfo(peer)->give_role) break;
							rtvar var = rtvar::parse(cch);
							string btn = var.get("buttonClicked");
							if (btn == "cfirm") {
								int itemid = atoi(var.get("itemid").c_str()), count = atoi(var.get("count").c_str()), ngems = atoi(var.get("gems").c_str());
								if (itemid <= 0 || itemid >= items.size()) {
									commands::setnewbie(peer);
									variants::OnTextOverlay(peer, "please input a valid itemid!");
									break;
								}
								else if (count <= 0 || count >= 201) {
									commands::setnewbie(peer);
									variants::OnTextOverlay(peer, "please input a valid count!");
									break;
								}
								else if (ngems <= 0 || ngems >= 10000000) {
									commands::setnewbie(peer);
									variants::OnTextOverlay(peer, "please input a valid gems amount!");
									break;
								}

								servers::newbie_award[itemid] = count;
								servers::newbieGems = ngems;
								servers::save_newbie();
								variants::OnConsoleMessage(peer, "Succes setting newbie award!");
								variants::OnTextOverlay(peer, "Succes setting newbie award!");
								break;
							}
							else if (btn.substr(0, 2) == "n_") {
								int itemid = atoi(btn.substr(2).c_str());
								if (itemid <= 0 || itemid >= items.size()) break;
								if (!servers::newbie_award.contains(itemid)) break;

								servers::newbie_award.erase(itemid);
								servers::save_newbie();
								variants::OnConsoleMessage(peer, "Succes delete: " + items[itemid].name + " from newbie award!");
								variants::OnTextOverlay(peer, "Succes delete: " + items[itemid].name + " from newbie award!");
								break;
							}

							break;
						}
						else if (cch.find("action|dialog_return\ndialog_name|setupgrade") != string::npos) {
							if (!pInfo(peer)->give_role) break;
							rtvar var = rtvar::parse(cch);
							string btn = var.get("buttonClicked");
							if (btn.substr(0, 4) == "del_") {
								int index = atoi(btn.substr(4).c_str());
								if (index >= servers::itemlist.size()) {
									variants::OnConsoleMessage(peer, "Invalid index. Cannot delete.");
									break;
								}

								servers::itemlist.erase(servers::itemlist.begin() + index);
								variants::OnConsoleMessage(peer, "Successfully delete item from upgrade menu");
								servers::save_upgrade();
								break;
							}
							else if (btn == "cfirm") {
								int id1 = atoi(var.get("id1").c_str()), c1 = atoi(var.get("c1").c_str());
								int id2 = atoi(var.get("id2").c_str()), c2 = atoi(var.get("c2").c_str());
								int id3 = atoi(var.get("id3").c_str()), c3 = atoi(var.get("c3").c_str());
								int toid = atoi(var.get("toid").c_str()), toam = atoi(var.get("toam").c_str());
								if (id1 >= items.size() || id2 >= items.size() || id3 >= items.size() || toid >= items.size()
									|| id1 <= 0 || id2 <= 0 || id3 <= 0 || toid <= 0) {
									commands::setupgrade(peer);
									variants::OnTextOverlay(peer, "please input a valid itemid!");
									break;
								}
								if (c1 <= 0 || c2 <= 0 || c3 <= 0 || toam <= 0
									|| c1 >= 201 || c2 >= 201 || c3 >= 201 || toam >= 201) {
									commands::setupgrade(peer);
									variants::OnTextOverlay(peer, "please input a valid amount!");
									break;
								}
								vector<pair<int, int>> newItem = { {id1, c1}, {id2, c2}, { id3, c3 }, {toid, toam} };
								bool match = false;
								for (const auto& entry : servers::exchange) {
									if (entry == newItem) {
										match = true;
										break;
									}
								...
								if (match) {
									commands::setupgrade(peer);
									variants::OnTextOverlay(peer, "Thats items already in /upgrade...");
									break;
								}
								else {
									servers::itemlist.push_back(newItem);
									servers::save_upgrade();
									commands::setupgrade(peer);
									variants::OnConsoleMessage(peer, "Succesfully added " + items[toid].name + " to upgrade");
									variants::OnTextOverlay(peer, "Succesfully added " + items[toid].name + " to upgrade");
									break;
								}
								break;
							}
						...
						else if (cch.find("action|dialog_return\ndialog_name|setexchange") != string::npos) {
							if (!pInfo(peer)->give_role) break;
							rtvar var = rtvar::parse(cch);
							string btn = var.get("buttonClicked");
							if (btn == "knt1") commands::setexchange(peer, btn);
							else if (btn == "knt2") commands::setexchange(peer, btn);
							else if (btn.substr(0, 4) == "del_") {
								int index = atoi(btn.substr(4).c_str());
								if (index >= servers::exchange.size()) {
									variants::OnConsoleMessage(peer, "Invalid index. Cannot delete.");
									break;
								}

								servers::exchange.erase(servers::exchange.begin() + index);
								variants::OnConsoleMessage(peer, "Successfully delete item from exchange");
								servers::save_exchange();
								break;
							}
							else if (btn == "cfirm") {
								int fromid = atoi(var.get("fromid").c_str()), toid = atoi(var.get("toid").c_str());
								if (fromid >= items.size() || toid >= items.size()) {
									commands::setexchange(peer, "knt1");
									variants::OnTextOverlay(peer, "please input a valid itemid!!");
									break;
								}
								int fromcount = atoi(var.get("fromcount").c_str()), tocount = atoi(var.get("tocount").c_str());
								if (fromcount >= 201 || tocount >= 201) {
									commands::setexchange(peer, "knt1");
									variants::OnTextOverlay(peer, "please input a valid amount, 1-200!!");
									break;
								}

								if (fromid <= 0 && toid <= 0 && fromcount <= 0 && tocount <= 0) {
									servers::isActive = (var.get("isactive") == "1" ? true : false);
									servers::save_exchange();
									variants::OnConsoleMessage(peer, "You has " + a + (servers::isActive ? "Active" : "Deactive") + " exchange for player");
									break;
								}
								else {
									vector<pair<int, int>> newItem = { {fromid, fromcount}, {toid, tocount} };
									bool match = false;
									for (const auto& entry : servers::exchange) {
										if (entry == newItem) {
											match = true;
									...
									if (match) {
										commands::setexchange(peer, "knt1");
										variants::OnTextOverlay(peer, "Thats items already in exchange...");
										break;
									}
									else {
										servers::exchange.push_back(newItem);
										commands::setexchange(peer, "knt1");
										variants::OnTextOverlay(peer, "Succesfully add " + items[fromid].name + " -> " + items[toid].name + " to exchange");
										variants::OnConsoleMessage(peer, "Succesfully add " + items[fromid].name + " -> " + items[toid].name + " to exchange");
										servers::save_exchange();
									}
								...
								bool isWl = (var.get("242") == "1" ? true : false);
								bool isDl = (var.get("1796") == "1" ? true : false);
								bool isBgl = (var.get("7188") == "1" ? true : false);
								int total = isWl + isDl + isBgl;
								if (total > 1 || total <= 0) {
									commands::setexchange(peer, "knt2");
									variants::OnTextOverlay(peer, "Please select one world lock type");
									break;
								}
								int price = atoi(var.get("price").c_str());
								if (price <= 0 || price >= 201) {
									commands::setexchange(peer, "knt2");
									variants::OnTextOverlay(peer, "Please input price 1-200");
									break;
								}
								servers::fishPrice = price;
								servers::type = (isWl ? "wl" : isDl ? "dl" : isBgl ? "bgl" : "wl");
								servers::save_exchange();
								variants::OnConsoleMessage(peer, "Succesfully edit sellfish");
								break;
							}

							break;
						}
						else if (cch.find("action|dialog_return\ndialog_name|upgrade_menu") != string::npos) {
							if (pInfo(peer)->level <= 25) break;
							rtvar var = rtvar::parse(cch);
							string btn = var.get("buttonClicked");
							if (btn.substr(0, 4) == "get_") {
								int index = atoi(btn.substr(4).c_str());
								if (index >= servers::itemlist.size()) break;
								vector<pair<int, int>> item = servers::itemlist[index];
								if (item.size() < 4) break;
								int from1 = item[0].first, from2 = item[1].first, from3 = item[2].first, toitem = item[3].first;
								int c1 = inventory_contains(peer, from1), c2 = inventory_contains(peer, from2), c3 = inventory_contains(peer, from3), toc = inventory_contains(peer, toitem);
								int am1 = item[0].second, am2 = item[1].second, am3 = item[2].second, toam = item[3].second;
								if (get_free_slots(pInfo(peer)) >= 2) {
									if (c1 < am1 || c2 < am2 || c3 < am3) {
										variants::OnConsoleMessage(peer, "You don't complete upgrade item needed");
										variants::OnTextOverlay(peer, "You don't complete upgrade item needed");
										break;
									}
									if (toc + toam >= 200) {
										variants::OnTextOverlay(peer, "You don't have more slot for " + items[item[3].first].name);
										variants::OnConsoleMessage(peer, "You don't have more slot for " + items[item[3].first].name);
										break;
									}
									
									if (modify_inventory(peer, from1, am1 *= -1) != -1 && modify_inventory(peer, from2, am2 *= -1) != -1 && modify_inventory(peer, from3, am3 *= -1) != -1) {
										modify_inventory(peer, toitem, toam);
										commands::upgrade(peer);
										variants::OnConsoleMessage(peer, "`6Upgrade: `0Succes upgrade to " + items[toitem].name);
										break;
									}
								}
								else {
									variants::OnConsoleMessage(peer, "You don't more have backpack slot");
									break;
								}
								break;
							}
						...
						else if (cch.find("action|dialog_return\ndialog_name|exchange") != string::npos) {
							if (pInfo(peer)->level <= 10) break;
							rtvar var = rtvar::parse(cch);
							string btn = var.get("buttonClicked");
							if (btn.substr(0, 4) == "get_") {
								int index = atoi(btn.substr(4).c_str());
								if (index >= servers::exchange.size()) break;
								vector<pair<int, int>> item = servers::exchange[index];
								if (item.size() < 2) break;
								int from = inventory_contains(peer, item[0].first), to = inventory_contains(peer, item[1].first);
								if (from >= 251 || to >= 251 || from <= 0) break;
								int c1_ = item[0].second, c2_ = item[1].second;

								if (get_free_slots(pInfo(peer)) >= 2) {
									if (from < c1_) {
										variants::OnTextOverlay(peer, "You don't enough " + items[item[0].first].name + ", You need "
											+ to_string(c1_ - from) + " More");
										variants::OnConsoleMessage(peer, "You don't enough " + items[item[0].first].name + ", You need "
											+ to_string(c1_ - from) + " More");
										break;
									}
									else {
										if (to + c2_ >= 200) {
											variants::OnTextOverlay(peer, "You don't have more slot for " + items[item[1].first].name);
											break;
										}
										else {
											if (modify_inventory(peer, item[0].first, c1_ *= -1) != -1) {
												modify_inventory(peer, item[1].first, c2_);
												commands::exchange(peer);
												//variants::OnTextOverlay(peer, "Succesfully");
												variants::OnConsoleMessage(peer, "`6Exchange: `0Succes exchange x" + to_string(item[0].second) + " " + items[item[0].first].name +
													" to x" + to_string(item[1].second) + " " + items[item[1].first].name);
												break;
											}
											else {
												variants::OnTextOverlay(peer, "Some wrong in your inventory");
												break;
											}
										}
									}
									...
									variants::OnTextOverlay(peer, "OOPS, you don't have more inventory slot..");
									break;
								}
						...
						else if (cch.find("action|dialog_return\ndialog_name|sellfish") != string::npos) {
							if (pInfo(peer)->level <= 10) break;
							rtvar var = rtvar::parse(cch);
							string btn = var.get("buttonClicked");
							if (btn == "sell_selected") {
								regex regex{ "^\\d+\\|\\d+$" };
								int outPuts = 0;
								...
								for (string const& line : explode("\n", cch)) {
									if (std::regex_match(line, regex)) {
										...
										int itemid = atoi(contents[0].c_str());
										if (items[itemid].blockType != BlockTypes::FISH) continue;
										if (contents[1] == "1") {
											int lbs = inventory_contains(peer, itemid);
											if (modify_inventory(peer, itemid, lbs *= -1) == 0) outPuts++;
										}
									}
								}
								if (outPuts != 0) {
									int price = servers::fishPrice, Locks = (servers::type == "dl" ? 100 : servers::type == "bgl" ? 10000 : 1);
									int total_wl = ((price * Locks) *outPuts), c_ = 0, taken = 0;
									...
									}
									{
										gamepacket_t p, p2;
										p.Insert("OnTalkBubble"), p.Insert(pInfo(peer)->netID);
										p.Insert((collect ? "You got " + collected_ + " from /sellfish" : "You don't have room in your backpack!"));
										p.Insert(0), p.Insert(1), p.CreatePacket(peer);
										p2.Insert("OnConsoleMessage");
										p2.Insert((collect ? "You got " + collected_ + " from /sellfish" : "You don't have room in your backpack!"));
										p2.CreatePacket(peer);
									}
								}
								break;
							}
							else if (btn == "sell_all") {
								regex regex{ "^\\d+\\|\\d+$" };
								int outPuts = 0;
						...
						else if (cch.find("action|dialog_return\ndialog_name|setitem") != string::npos) {
							if (!pInfo(peer)->give_role) break;
							rtvar var = rtvar::parse(cch);
							string btn = var.get("buttonClicked");
							if (btn == "cfirm") {
								int itemid = atoi(var.get("itemid").c_str()), gems = atoi(var.get("gems").c_str()),
									exp = atoi(var.get("exp").c_str()), fars = atoi(var.get("fars").c_str()), hit = atoi(var.get("hit").c_str());
								if (itemid <= 0 || itemid >= items.size()) {
									commands::setitem(peer);
									variants::OnTextOverlay(peer, "Please Input a valid itemid!");
									break;
								}
								else if (gems <= 0 || gems >= 1000) {
									commands::setitem(peer);
									variants::OnTextOverlay(peer, "Please Input a valid (x) gems!");
									break;
								}
								else if (exp <= 0 || exp >= 1000) {
									commands::setitem(peer);
									variants::OnTextOverlay(peer, "Please Input a valid (x) exp!");
									break;
								}
								else if (fars <= 0 || fars >= 30) {
									commands::setitem(peer);
									variants::OnTextOverlay(peer, "Please Input a valid fars item, 1-30!");
									break;
								}
								else if (hit <= 0 || hit >= 5) {
									commands::setitem(peer);
									variants::OnTextOverlay(peer, "Please Input a valid breakhits, 1-5!");
									break;
								}
								
								servers::fars[itemid] = fars, servers::gems[itemid] = gems,
									servers::exp[itemid] = exp, servers::breakhits[itemid] = hit;
								servers::save_itemadjustment();
								variants::OnConsoleMessage(peer, "Succes Edit Item: " + items[itemid].name);
								break;
							}
							else if (btn.substr(0, 4) == "far_") {
								int itemid = atoi(btn.substr(4).c_str());
								if (itemid <= 0 || itemid >= items.size()) break;
								if (!servers::fars.contains(itemid)) break;

								servers::fars.erase(itemid);
								servers::save_itemadjustment();
								commands::setitem(peer);
								variants::OnConsoleMessage(peer, "Removed Fars Effect: " + items[itemid].name);
								variants::OnTextOverlay(peer, "Removed Fars Effect: " + items[itemid].name);
								break;
							}
							else if (btn.substr(0, 5) == "gems_") {
								int itemid = atoi(btn.substr(5).c_str());
								if (itemid <= 0 || itemid >= items.size()) break;
								if (!servers::gems.contains(itemid)) break;

								servers::gems.erase(itemid);
								servers::save_itemadjustment();
								commands::setitem(peer);
								variants::OnConsoleMessage(peer, "Removed (x) Gems Effect: " + items[itemid].name);
								variants::OnTextOverlay(peer, "Removed (x) Gems Effect: " + items[itemid].name);
								break;
							}
							else if (btn.substr(0, 4) == "exp_") {
								int itemid = atoi(btn.substr(4).c_str());
								if (itemid <= 0 || itemid >= items.size()) break;
								if (!servers::exp.contains(itemid)) break;

								servers::exp.erase(itemid);
								servers::save_itemadjustment();
								commands::setitem(peer);
								variants::OnConsoleMessage(peer, "Removed (x) Exp Effect: " + items[itemid].name);
								variants::OnTextOverlay(peer, "Removed (x) Exp Effect: " + items[itemid].name);
								break;
							}
							else if (btn.substr(0, 4) == "hit_") {
								int itemid = atoi(btn.substr(4).c_str());
								if (itemid <= 0 || itemid >= items.size()) break;
								if (!servers::breakhits.contains(itemid)) break;

								servers::breakhits.erase(itemid);
								servers::save_itemadjustment();
								commands::setitem(peer);
								variants::OnConsoleMessage(peer, "Removed Breakhits Effect: " + items[itemid].name);
								variants::OnTextOverlay(peer, "Removed Breakhits Effect: " + items[itemid].name);
								break;
							}

							break;
						}
						else if (cch.find("action|dialog_return\ndialog_name|erole") != string::npos) {
							if (!pInfo(peer)->give_role && pInfo(peer)->tankIDName != "Zenhyx" && pInfo(peer)->last_wrenched.empty()) break;
							rtvar parser = rtvar::parse(cch);
							for (ENetPeer* currentPeer = server->peers; currentPeer < &server->peers[server->peerCount]; ++currentPeer) {
								if (currentPeer->state != ENET_PEER_STATE_CONNECTED or currentPeer->data == NULL) continue;
								if (to_lower(pInfo(currentPeer)->tankIDName) == to_lower(pInfo(peer)->last_wrenched)) {
									if (parser.get("vip") == "1") add_playmod(currentPeer, 126), pInfo(currentPeer)->vip = 1;
									else remove_playmod(pInfo(currentPeer), 126), pInfo(currentPeer)->vip = 0;
									if (parser.get("mod") == "1") pInfo(currentPeer)->mod = 1, pInfo(currentPeer)->moderator = 1;
									else pInfo(currentPeer)->mod = 0, pInfo(currentPeer)->moderator = 1;
									if (parser.get("dev") == "1") pInfo(currentPeer)->dev = 1;
									else pInfo(currentPeer)->dev = 0;
									if (parser.get("superdev") == "1") pInfo(currentPeer)->superdev = 1;
									else pInfo(currentPeer)->superdev = 0;
									if (parser.get("staff") == "1") pInfo(currentPeer)->Staff = 1, pInfo(currentPeer)->custom_name_color = "`4@";
									else pInfo(currentPeer)->Staff = 0, pInfo(currentPeer)->custom_name_color.clear();
									if (parser.get("owner") == "1") pInfo(currentPeer)->give_role = 1;
									else pInfo(currentPeer)->give_role = 0;

									if (parser.get("cheater") == "1") pInfo(currentPeer)->cheater_ = 1;
									else pInfo(currentPeer)->cheater_ = 0;
									if (parser.get("ltit") == "1") pInfo(currentPeer)->drlegend = 1;
									else pInfo(currentPeer)->drlegend = 0;
									if (parser.get("drtitle") == "1") pInfo(currentPeer)->drtitle = 1;
									else pInfo(currentPeer)->drtitle = 0;
									if (parser.get("gpass") == "1") pInfo(currentPeer)->gp = 1;
									else pInfo(currentPeer)->gp = 0;
									if (parser.get("ssup") == "1") pInfo(currentPeer)->supp = 2;
									else pInfo(currentPeer)->supp = 0;
						...
						else if (cch.find("action|dialog_return\ndialog_name|find_item") != string::npos) {
							if (!pInfo(peer)->Staff) break;
							try {
								rtvar parser = rtvar::parse(cch);
								vector<string> a_ = explode("|", cch);
								if (a_.size() < 5) break;
								if (cch.find("type|") == string::npos) break;

								std::string value = parser.get("type"), btn = explode("\n", a_[5])[0];
								if (value.empty() || special_char(value)) break;

								if ((btn.substr(0, 5) == "next_" || btn.substr(0, 5) == "prev_") && value == "searched") {
									vector<string> i_ = explode("_", btn);
									if (i_.size() < 1) break;
									string targetpage = i_[1], cpage = i_[2];
										...
										if (std::regex_match(line, regex)) {
											...
											if (itemId >= 0 && itemId <= items.size()) {
												if (contents[1] == "1") {
													...
													if (items[itemId].blockType == BlockTypes::LOCK && !pInfo(peer)->give_role) continue;
													if (items[itemId].blockType == BlockTypes::CLOTHING && !pInfo(peer)->Staff) continue;

													if (Allowed) pInfo(peer)->listFindItems.insert(itemId);
												}
									...
									send_find_dialog(peer, targetpage, (btn.substr(0, 5) == "prev_" ? atoi(cpage.c_str()) - 1 : atoi(cpage.c_str()) + 1), true);
									break;
								}
								else if (btn == "find_name" && value == "search") send_find_dialog(peer, parser.get("name"));
								...
								if (value == "searched" && btn == "claim") {
									if (pInfo(peer)->listFindItems.empty()) {
										console_msg(peer, "`o>> You haven't select your finded items.");
										break;
									}
									...
									for (int const& itemId : pInfo(peer)->listFindItems) {
										if (itemId >= 0 && itemId <= items.size()) {
											int quantity = 200;

											if (itemId != invalid) {
												itemReceived += "\nadd_label_with_icon|small|`$" + to_string(quantity) + " " + items[itemId].name + " `$(`rYou are developer`$)|left|" + to_string(items[itemId].id) + "|";
											}
										}
									}
									...
									itemDialogs += "\nadd_button|purchasing|`wPurchase Items|noflags|0|0|";
									itemDialogs += "\nend_dialog|find_item|`wCancel||";

									gamepacket_t p(250);
									p.Insert("OnDialogRequest");
									p.Insert(itemDialogs);
									p.CreatePacket(peer);
									break;
								}
								else if (value == "purchase" && btn == "purchasing") {
									if (pInfo(peer)->listFindItems.empty()) {
										console_msg(peer, "`o>> You haven't select your finded items.");
										break;
									}
									...
									std::string itemReceived = "", logsReceived;

									for (int const& itemId : pInfo(peer)->listFindItems) {
										int quantity = 200;

										if (itemId != wrongId) {
											int c_ = inventory_contains(peer, itemId), result = quantity - c_;
											if (modify_inventory(peer, itemId, result) != -1) {
												itemReceived += "\nadd_label_with_icon|small|`$" + to_string(result) + " " + items[itemId].name + " `otaken from /find.|left|" + to_string(items[itemId].id) + "|";
											}
										}
									}
									...
									itemDialogs += "\nadd_button|find_name|`wFind Items|noflags|0|0|";
									itemDialogs += "\nend_dialog|find_item|`wCancel||";
							...
							for (int i_ = 0; i_ < world_->drop_new.size(); i_++) {
								if (world_->drop_new[i_][2] == p_->plantingTree) {
									...
									if (noclipas == false) {
										...
										bool noclip = false;
										if (pInfo(peer)->Staff || to_lower(world_->owner_name) == to_lower(pInfo(peer)->tankIDName) || find(world_->admins.begin(), world_->admins.end(), to_lower(pInfo(peer)->tankIDName)) != world_->admins.end()) {
											if (pInfo(peer)->Staff || to_lower(world_->owner_name) == to_lower(pInfo(peer)->tankIDName) || find(world_->admins.begin(), world_->admins.end(), to_lower(pInfo(peer)->tankIDName)) != world_->admins.end()) displaybox = true;
										}
										if (displaybox) {
											int c_ = world_->drop_new[i_][1];
